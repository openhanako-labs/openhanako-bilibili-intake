...
import { canonicalId, findRec, readRecords, upsertRecord } from "../lib/records.js";
...
export async function execute(input = {}, ctx) {
  try {
    ...
    const record = locate(input, ctx);
    const slotDir = String(input.slotDir || record?.artifactDir || "").trim();
    if (!slotDir) {
    ...
    let recordId = summary.recordId;
    if (!recordId && record) recordId = record.id;
    if (recordId) {
      try {
        upsertRecord(ctx, {
          id: recordId,
          summary: summary.brief || undefined,
          summaryPath: `${slotDir}/summary.json`,
          summaryPoints: summary.counts.total,
          summaryGrounded: summary.counts.grounded,
          summaryUngrounded: summary.counts.ungrounded,
          summaryAt: summary.generatedAt,
          summaryModel: summary.model || undefined,
        });
      } catch { /* 记录回写失败不影响摘要落盘 */ }
    }
    ...
    return toToolResult(
      { ok: true, slotDir, recordId, counts: summary.counts, summaryPath: `${slotDir}/summary.json`, summary },
