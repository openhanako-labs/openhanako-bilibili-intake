...
/**
 * ⭐ W4（2026-09-26）：摘要 → 记录字段的**唯一一份**实现。
 *
 * 过去「工具侧回写」与「启动对账」是两处各写一遍字段名 —— 而本 App 已经踩过一次
 * 同类坑（records.js 的注释里记着：卡片侧与工具侧字段漂移会长出重复记录）。
 * 抽出来之后，加一个字段只改这里。
 *
 * 约定：brief 为空时不覆盖已有值（upsertRecord 自己会剔除空值）——
 * “手写总结优先级最高”这条规则仍然成立。
 */
export function patchFromSummary(summary, { slotDir, recordId } = {}) {
  const s = summary || {};
  const counts = s.counts || {};
  const patch = {
    summary: s.brief || undefined,
    summaryPath: slotDir ? summaryPath(slotDir) : undefined,
    summaryPoints: counts.total,
    summaryGrounded: counts.grounded,
    summaryUngrounded: counts.ungrounded,
    summaryAt: s.generatedAt || undefined,
    summaryModel: s.model || undefined,
  };
  const id = recordId || s.recordId || "";
  if (id) patch.id = id;
  return patch;
}
