...
export async function prepareRuntime(ctx, settings) {
  ...
  // ⭐ BUG-2：把 torch 安装失败从兜底报错里单独认出来，说真话并带下一步动作。
  // 兜底那句会把排查方向引向"Python 装坏了"，但真因常是 pip install torch 退出码非 0
  // (孤儿 torch 目录挡路 / 沙箱内 pip 失败)。单模式时 installTorch 抛出的错误已带完整指引
  // (见 buildTorchInstallFailure)，这里兜住多模式聚合后同样要认出它、不再落到笼统的兜底句。
  const torchFailure = failures.find(
    (f) => f?.code === "TORCH_INSTALL_FAILED"
      || f?.details?.cause?.code === "TORCH_INSTALL_FAILED"
      || f?.message?.includes("torch 运行时失败"),
  );
  if (torchFailure) {
    const orphanDir = torchFailure?.details?.orphanDir
      || venvSitePackagesTorch(torchFailure?.venvDir);
    const cpuCmd = `python -m pip install --index-url ${TORCH_CPU_INDEX_URL} torch`;
    throw new BiliIntakeError(
      "安装 Whisper 所需的 torch 运行时失败（pip install torch 未成功）。" +
      (orphanDir ? `若这是上一次安装被打断留下的半成品，请删除 ${orphanDir} 后重试。\n` : "") +
      `或在宿主持有写权限与网络的 shell 里执行: ${cpuCmd}，再让 App 重试。`,
      {
        ...
        details: {
          ...
          attempts: failures,
          orphanDir,
          hint: orphanDir ? `删除 ${orphanDir} 后重试` : cpuCmd,
        },
        cause: torchFailure,
      },
    );
...
async function ensurePythonEnvironment(runtime) {
  ...
  // ⭐ BUG-1 / BUG-3 修复：解释器存在 ≠ 环境可用。半成品 venv（上一次 pip install 被打断、
  //    在 site-packages 里留下的孤儿 torch 目录）会让 python.exe 在、但 import torch 失败。
  //    用一次真实 import 作为"环境可用"的证据，和 marker hash 一起构成就绪判据。
  //    queryTorchState 内部以 isVenvReady（解释器在）为前置，天然只应在此处调用，不会自调用死循环；
  //    探活失败(返回 null) → venvUsable=false → 继续走下面的 pip install 重装流程 ——
  //    installTorch 会先 pip uninstall torch 清掉孤儿目录再安装，可直接覆盖装成功。
  const torchState = await queryTorchState(runtime);
  const venvUsable = venvReady && !!(torchState && torchState.version);

  if (venvUsable && await isInstallSatisfied(runtime, installMarker, requirementsHash, torchPlan, torchState)) {
    return;
  }
...
export async function runCollector(runtime, payload, options = {}) {
  ...
  // ⭐ v0.6.38：CPU 转写的线程数（0 = 自动：核数一半；-1 = 不限制）。
  //   以前不传 —— faster-whisper 的 cpu_threads 默认 0 = 吃满所有核，一转写机器就卡。
  const wt = payload.whisperCpuThreads ?? runtime.settings.whisperCpuThreads;
  if (wt !== undefined && wt !== null && wt !== "") args.push("--whisper-cpu-threads", String(wt));
  ...
  // Mode
  // ⭐ 2026-09-26：把原片下下来（小红书视频笔记 -> output_dir/source_video.mp4，接帧分析）。
  if (payload.downloadVideo) {
    args.push("--download-video");
  }
  ...
  // 旧视觉链路已于 2026-09-26 下线（frame_extractor / visual_analyzer / vision_backend）。
  //   画面分析现在走 lib/shots + 宿主视觉通道，入口是工具 intake_shots；
  //   不再由采集命令带 --visual 一起跑，所以 --visual / --frame-* 的转发一并删掉。
...
async function isInstallSatisfied(runtime, installMarker, requirementsHash, torchPlan, knownTorchState) {
  ...
  // torch 到底能不能用，由 torchState 检查覆盖。
  // BUG-1：torchState 由调用方(ensurePythonEnvironment)探活后传入，避免对同一 venv 重复 import torch。
  const torchState = knownTorchState ?? await queryTorchState(runtime);
  ...
  for (const candidate of torchPlan.candidates) {
    try {
      ...
      if (!torchPlan.allowFallback || candidate === torchPlan.candidates.at(-1)) {
        throw buildTorchInstallFailure(runtime, torchPlan, failures, error);
      }
    }
  }

  throw buildTorchInstallFailure(runtime, torchPlan, failures);
}

/** venv 的 site-packages/torch 路径（用于在报错里指出孤儿目录；跨平台给近似值）。 */
function venvSitePackagesTorch(venvDir) {
  if (!venvDir) {
    return null;
  }
  if (process.platform === "win32") {
    return path.join(venvDir, "Lib", "site-packages", "torch");
  }
  return path.join(venvDir, "lib", "site-packages", "torch");
}

/**
 * BUG-2：构造一个说真话、自带下一步动作的 torch 安装失败错误。
 *
 * 兜底那句「自动选择 Python 运行环境失败」会把排查者引向"Python 装坏了"，但真因常常是
 * pip install torch 退出码非 0：常见是上一次安装被打断、site-packages 里留下半成品(孤儿)
 * torch 目录挡住覆盖安装，或 AppHost 沙箱内 pip 没有网/写权限。把 venvDir、孤儿目录、
 * 手动安装命令都写进 message 与 details，让用户照着就能修。
 */
function buildTorchInstallFailure(runtime, torchPlan, failures, cause) {
  const orphanDir = venvSitePackagesTorch(runtime?.venvDir);
  const cpuCmd = `python -m pip install --index-url ${TORCH_CPU_INDEX_URL} torch`;
  const hint = orphanDir
    ? `删除 ${orphanDir} 后重试；或手动执行: ${cpuCmd}`
    : `手动执行: ${cpuCmd}`;
  return new BiliIntakeError(
    "安装 Whisper 所需的 torch 运行时失败（pip install torch 未成功）。" +
    "常见原因是上一次安装被打断，venv 的 site-packages 里留下了一个半成品(孤儿)torch 目录，挡住覆盖安装。\n" +
    "解法(按序尝试)：\n" +
    (orphanDir ? `  1) 删除该目录后重试: ${orphanDir}\n` : "") +
    `  2) 手动安装: ${cpuCmd}\n` +
    "  3) 若 AppHost 沙箱内 pip 失败，请在宿主持有写权限与网络的 shell 里执行上面这条，再让 App 重试。",
    {
      code: "TORCH_INSTALL_FAILED",
      details: {
        preferredFamily: torchPlan?.preferredFamily ?? null,
        requiredFamily: torchPlan?.requiredFamily ?? null,
        venvDir: runtime?.venvDir ?? null,
        orphanDir,
        cpuIndexUrl: TORCH_CPU_INDEX_URL,
        hint,
        failures,
      },
      cause,
...
function spawnAndCollect(command, args, label, runtime, timeoutMs = SPAWN_TIMEOUT_MS) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      if (!settled) {
        ...
        reject(new BiliIntakeError(`执行 ${label} 超时（${timeoutMs / 1000}s）。`, {
          ...
          details: { command, args, timeout: timeoutMs, stdout, stderr },
        }));
      }
    }, timeoutMs);
    ...
    child.on("error", (error) => {
      settle(() => reject(new BiliIntakeError(`执行 ${label} 失败：${error.message}`, {
        code: "COMMAND_SPAWN_FAILED",
        details: { command, args, stderr },
        cause: error,
      })));
    });
    child.on("close", (code) => {
      settle(() => {
        ...
        // ⭐ 2026-09-26：权限类失败单独点名 —— 宿主的 --permission 会拦 spawn/文件访问，
        //   纯看退出码会把人往 Python 装坏了的方向带（这个坑今天真踩过）。
        const denied = /allow-child-process|allow-fs-|Access to this API has been restricted/i
          .test(String(stderr));
        reject(new BiliIntakeError(
          `执行 ${label} 退出码 ${code}。` + (denied ? "（疑似宿主权限模型拒绝：--allow-child-process / --allow-fs-*）" : ""),
          {
            code: denied ? "SPAWN_PERMISSION_DENIED" : "COMMAND_FAILED",
            details: { command, args, code, stdout, stderr },
          },
        ));
      });
    });
